...
arr3[1,3]  # Same as above
arr3[:2]  # 1st and 2nd rows
arr3[2:]  # 3rd row
arr3[1:,:3]  # 2nd-3rd rows and 1st-3rd columns
arr3[:,2]  # 3rd Column 
...
np.sum((arr4 < 2) & (arr4 > -2)) # Count entries between -2 and 2; note &
...
# If we want the "grid" of rows indexed 2,4,5 and columns indexed 0,1,3:
arr5[[2,4,5]][:,[0,1,3]]  # A bit awkward, but it works.
...
# This time we just replace elements between 10 and 20
# (inclusive) with 100
np.where((arr5 >= 10) & (arr5 <= 20), 100, arr5) # parens important!

# Here we change the sign of elements less than 20, keeping
# the others the same.
np.where(arr5 < 20, -arr5, arr5)
...
np.median(arr5, axis=1)  # The median of each row
...
np.setxor1d(arr6,arr7)  # Elements in exactly one of arr6 and arr7
